...
/-- https://docs.python.org/3/reference/lexical_analysis.html#operators -/
def operator (startPos : String.Pos) : TokenizerM (TkResult startPos) := do
  let input ← getInput
  let subStr := input.extract startPos (startPos + ⟨3⟩)
  let res : Option (TokenKind × PosGt startPos) :=
         if subStr.startsWith ":=" then some (.colonassign, ⟨startPos + ":=", String.add_gt startPos _ (by simp_str_size)⟩)
...
/-- https://docs.python.org/3/reference/lexical_analysis.html#delimiters -/
def delimiter (startPos : String.Pos) : TokenizerM (TkResult startPos) := do
  let input ← getInput
  let subStr := input.extract startPos (startPos + ⟨3⟩)
  let res : Option (TokenKind × PosGt startPos) :=
...
def finishLongStr (startPos : String.Pos) (firstQuote : Char) : TokenizerM (String × PosGe startPos) := do
  ...
  let rec go (s : String) (startPos : String.Pos) : TokenizerM (String × PosGe startPos) := do
    ...
    if tquote == input.extract startPos (startPos + tquote) then pure (s, ⟨startPos, by simp⟩) else
...
def runTokenizer (input : String) (fileName : String) (fileMap : FileMap := input.toFileMap) : Except String (Array Token) :=
...
namespace test
  ...
  def test (s : String) : String :=
    let tokens := runTokenizer s "<input>"
    match tokens with
