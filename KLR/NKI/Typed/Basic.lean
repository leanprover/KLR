...
instance : EmptyCollection Kind := ⟨.unknown {}⟩
...
inductive Typ
  ...
  | forall (span : Span) (names : List Ident) (body : Typ)
...
inductive Exp
  ...
  /-- Stores the result type of the call in `typ`. -/
...
where
  ...
  | .var _ v             => s.insert v
  | .evar _ v            => s.insert v
  | .forall _ names body => names.foldl (fun s n => s.erase n) (visit s body)
  | .prim _ _            => s
...
def Typ.toString : Typ → String
  | .var _ name          => name
  | evar _ name          => s!"?{name}"
  | .forall _ names body => "∀ " ++ (" ".intercalate names) ++ ". " ++ body.toString
  | .prim _ p            => p.toString
  | .func _ params ret   => s!"func[{", ".intercalate ((params.map Typ.toString).concat ret.toString)}]"
  | .size _ n            => s!"{n}"
  ...
  | .tensor _ sh dt      => s!"tensor[{dt.toString}, {sh.toString}]"
