...
--   let coeff_sum : Int := (p.pattern.map APPair.step).sum
--   Layout.mk
--     (p.freeOffset + coeff_sum * l.offset).toNat
--     (l.steps.map (coeff_sum * ·))
--     (p.pattern.map APPair.num)
--     l.offsets
...
--   tensor := t
--   parNum := parNum
--   pattern := (List.zip (List.zip l.steps l.nums) l.offsets).map (fun ((step, num), offset) => ⟨step, num, offset⟩)
--   freeOffset := l.offset
--
-- def Layout.rowMajorForm (s : Shape) : Layout s.freeDim where
--   offset := 0
--   offsets := List.replicate s.freeDim 0
...
def idxToAp (layout : List Nat) (idxs : List Index) : List APPair :=
  ...
  let pairs : List APPair := List.zipWith (fun s (cnt, off) => ⟨s, cnt.toNat, off.toNat⟩) steps sz_off
  pairs

def AccessBasic.toAP (a : AccessBasic) : KLR.Err AccessPattern := do
  let layout := a.tensor.shape.toList
  let pairs := idxToAp layout a.indexes
  return {
    tensor := a.tensor
    parNum := pairs[0]!.num
    pattern := pairs
...
def Access.combineAP (a : Access) (pat: List APPair) : KLR.Err AccessPattern := do
  let ap <- Access.toAP a
  let pat1 := ap.pattern
  let pairs := List.zipWith (fun p1 p2 => APPair.combine p1 p2) pat1 pat
  return {
    tensor := ap.tensor
    parNum := pairs[0]!.num
    pattern := pairs
    freeOffset := pairs.foldl (fun acc p => acc + p.offset) 0
  }
...
def Access.normWithAP (fixedAxis: List Nat) (this : List APPair) (pairs : List APPair) : List APPair :=
  let rvPairs := pairs.mapIdx fun i pair =>
    if fixedAxis.contains i then
      { step := pair.step, num := 1, offset := 0 }
    else
      this[i - (fixedAxis.filter (· < i)).length]!
  rvPairs
...
def Access.combine (a : Access) (idx : List Index) : KLR.Err AccessPattern := do
  ...
  let ap <- Access.toAP a
  let shape := <- a.shape
  dbg_trace s!"{repr shape}"
  let pat2 := idxToAp shape.toList idx
  dbg_trace s!"idx {repr idx} to pairs {repr pat2}"
  let pat2 := Access.normWithAP ap.fixedAxis pat2 ap.pattern
  dbg_trace s!"pairs after norm {repr pat2}"
  Access.combineAP a pat2
...
#eval testAccess2
    ...
    [.slice (Slice.make! 0 2 1), .slice (Slice.make! 0 2 1)]
...
#guard testAccess2
    [.slice (Slice.make! 1 2 1), .slice (Slice.make! 0 3 1), .slice (Slice.make! 0 2 1)]
    [.coord 0, .slice (Slice.make! 1 2 1), .slice (Slice.make! 0 2 1)] ==
    .ok ([⟨6,1,1⟩, ⟨2,1,0⟩, ⟨1,2,0⟩], 8)
#guard testAccess2
    [.slice (Slice.make! 1 3 1), .slice (Slice.make! 1 3 1), .slice (Slice.make! 0 2 1)]
    [.slice (Slice.make! 0 2 1), .slice (Slice.make! 1 2 1), .coord 1] ==
     .ok ([⟨6,2,1⟩, ⟨2,2,1⟩, ⟨1,1,1⟩], 11)
#guard testAccess2
    [.slice (Slice.make! 1 3 1), .slice (Slice.make! 1 3 1), .coord 1]
    [.slice (Slice.make! 0 2 1), .slice (Slice.make! 0 2 1)] ==
     .ok ([⟨6,2,1⟩, ⟨2,2,1⟩, ⟨1,1,1⟩], 11)
