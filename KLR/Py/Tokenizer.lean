...
def newline (pos : String.Pos) : TokenizerM (Option (PosGt pos)) := do
  let input ← getInput
  ...
  match next with
  | some next =>
    if !(← lineJoin) then pushToken .newline ⟨pos, next.val⟩
    return next
  | none => return none
...
def finishLineComment (pos : String.Pos) : TokenizerM (PosGt pos) := do
  let input ← getInput
...
def checkIndent (startPos : String.Pos) : TokenizerM (CheckIndentResult startPos) := do
  ...
  | '#' =>
    let next ← finishLineComment next
...
def finishStr (pos : String.Pos) (doubleQuote : Bool) : TokenizerM (String × PosGt pos) := do
  ...
  if (input.extract pos input.endPos).startsWith dquote then
    let next : PosGt pos := ⟨pos + dquote, String.add_gt (by simp only [dquote]; split <;> simp_str_size)⟩
    let (s, endPos) ← finishLongStr pos doubleQuote
