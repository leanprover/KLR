...
def generateCommonAST: MetaM Unit := do
  ...
  for t in [.bool, .nat, .int, .float, .string] do
    genList t.name t
  genTypes (<- commonAST)
...
def generatePythonAST : MetaM Unit := do
  let tys <- pythonAST
  IO.println headerH
  IO.println "#include \"ast_common.h\""
...
def generateNkiAST : MetaM Unit := do
  ...
  genTypes tys
  for n in ["Expr", "Index", "Keyword", "Stmt", "Fun", "Arg", "Param"] do
    let n := .str `KLR.NKI n
    genList n (.const n)
