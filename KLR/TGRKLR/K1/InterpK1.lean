...
namespace KLR.TGRKLR.K1.Interp
...
inductive Value where
  ...
  | int (n : Int)
  | float (f : Float32)
  | address (memory : Core.SramMemory) (parOffset : Nat) (freeOffset : Nat)
...
def interpScalar (scalar : ScalarK1) : Interp Value := do
  ...
  | .reg reg => do
    ...
    | .some v => pure v
  | .address memory parOffset freeOffset =>
    let parQuadrant := parQuadrantToNat parOffset
  ...
  | .tensorScalar ⟨dst, src, imm0, op0, _imm1, _op1, _reverse⟩ =>
    ...
    | .reg r =>
      ...
      | .address memory parOffset freeOffset =>
        ...
        let vec := { name := "", dtype := src.dtype, memory := memory, parQuadrant := .par0, parDim, freeOffset, freePattern := [⟨1, 1⟩] }
