...
def runLncKernels (k : NKI.Kernel) (genDebug : Bool := false)
  ...
  let allOutputNames := outputLists.flatMap (·.map (·.name))
  let filteredSharedBuffers := sharedBuffers.filter fun (t, _) =>
    !allOutputNames.contains t.name
  let dedupedSharedBuffers <- if num > 1 then dedupSharedBuf filteredSharedBuffers else pure (sharedBuffers.map (·.1))
