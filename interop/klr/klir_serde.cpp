...
Ptr<Operator> Operator_des(FILE *in) {
  ...
  case 7: {
    if (l != 1)
      throw std::runtime_error("Wrong number of elements");
    Ptr<OperatorDmaHbmLoadWrapper> x = ptr<OperatorDmaHbmLoadWrapper>();
    x->op = DmaHbmLoad_des(in);
    return x;
    break;
  }
  case 8: {
    if (l != 1)
      throw std::runtime_error("Wrong number of elements");
    Ptr<OperatorDmaHbmStoreWrapper> x = ptr<OperatorDmaHbmStoreWrapper>();
    x->op = DmaHbmStore_des(in);
    return x;
    break;
  }
  case 9: {
...
Ptr<Kernel> Kernel_des(FILE *in) {
  ...
  x->name = String_des(in);
  x->inputs = List_TensorSram_des(in);
  x->outputs = List_TensorSram_des(in);
...
List<Ptr<TensorSram>> List_TensorSram_des(FILE *in) {
  ...
  List<Ptr<TensorSram>> l;
  while (size-- > 0) {
    Ptr<TensorSram> b = TensorSram_des(in);
    l.push_back(b);
  }
  return l;
}
