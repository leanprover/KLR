...
bool Core_DmaHbmLoad_ser(FILE *out, struct Core_DmaHbmLoad *x) {
  ...
  if (!Core_TensorRef_ser(out, x->dst))
    return false;
  if (!Core_TensorHbm_ser(out, x->src))
    return false;
  if (!Core_DgeComputeOp_ser(out, x->compute_op))
    return false;
  if (!Core_DmaBounds_ser(out, x->dstBoundsCheck))
...
bool Core_Kernel_ser(FILE *out, struct Core_Kernel *x) {
  ...
  if (!String_ser(out, x->name))
    return false;
  if (!Core_TensorSram_List_ser(out, x->inputs))
    return false;
  if (!Core_TensorSram_List_ser(out, x->outputs))
    return false;
...
bool Core_TensorSram_List_ser(FILE *out, struct Core_TensorSram_List *x) {
  u64 count = 0;
  for (struct Core_TensorSram_List *node = x; node; node = node->next)
    count++;
  if (!cbor_encode_array_start(out, count))
    return false;
  for (struct Core_TensorSram_List *node = x; node; node = node->next)
    if (!Core_TensorSram_ser(out, node->tensorsram))
      return false;
  return true;
}
...
bool Core_DmaHbmLoad_des(FILE *in, struct region *region,
  ...
  *x = region_alloc(region, sizeof(**x));
  if (!Core_TensorRef_des(in, region, &(*x)->dst))
    return false;
  if (!Core_TensorHbm_des(in, region, &(*x)->src))
    return false;
  if (!Core_DgeComputeOp_des(in, region, &(*x)->compute_op))
    return false;
  if (!Core_DmaBounds_des(in, region, &(*x)->dstBoundsCheck))
...
bool Core_Operator_des(FILE *in, struct region *region,
  ...
  *x = region_alloc(region, sizeof(**x));
  ...
  case 7:
    if (l != 1)
      return false;
    if (!Core_DmaHbmLoad_des(in, region, &(*x)->dmaHbmLoad.op))
      return false;
    (*x)->tag = Core_Operator_dmaHbmLoad;
    break;
  case 8:
    if (l != 1)
      return false;
    if (!Core_DmaHbmStore_des(in, region, &(*x)->dmaHbmStore.op))
      return false;
    (*x)->tag = Core_Operator_dmaHbmStore;
    break;
  case 9:
    if (l != 1)
      return false;
...
bool Core_Kernel_des(FILE *in, struct region *region, struct Core_Kernel **x) {
  ...
  *x = region_alloc(region, sizeof(**x));
  if (!String_des(in, region, &(*x)->name))
    return false;
  if (!Core_TensorSram_List_des(in, region, &(*x)->inputs))
    return false;
  if (!Core_TensorSram_List_des(in, region, &(*x)->outputs))
    return false;
...
bool Core_TensorSram_List_des(FILE *in, struct region *region,
  ...
  if (!cbor_decode_array_start(in, &count))
    return false;
  struct Core_TensorSram_List *current = *x = NULL;
  for (; count > 0; count--) {
    struct Core_TensorSram_List *node = region_alloc(region, sizeof(*node));
    node->next = NULL;
    if (!current) {
      *x = current = node;
    } else {
    ...
    if (!Core_TensorSram_des(in, region, &node->tensorsram))
      return false;
  }
  return true;
}
