...
static struct ref reference_(struct state *st, struct Python_Expr *e) {
  // goto cleanup if anything goes wrong
  ...
  case Python_Expr_attr:
    attr_value_ref = reference_(st, e->expr->attr.value); // new reference
...
static void reference(struct state *st, struct Python_Expr *e) {
  struct ref ref = reference_(st, e); // new reference
  Py_XDECREF(ref.obj);
}
